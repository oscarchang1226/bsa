...
InlineQuizApp.BuildQuiz = function() {
    ...
    if (InlineQuizApp.onStart !== null && InlineQuizApp.onStart.constructor === Function) {
        startFunction = InlineQuizApp.onStart;
    } else {
        startFunction = InlineQuizApp.GoNextQuestion;
    }
...
InlineQuizApp.goEndSlide = function() {
    ...
    if (InlineQuizApp.QuizData) {
        if (InlineQuizApp.QuizData.General.repeatOnComplete) {
            InlineQuizApp.SetTextSlide(endText,
                function() {
                    ...
                    if (InlineQuizApp.onReset !== null && InlineQuizApp.onReset.constructor === Function) {
                        try {
                            InlineQuizApp.onReset();
                        } catch (err) {
                            d2log('onReset Error!');
                            d2log(err);
                        }
        ...
        if (InlineQuizApp.onPostComplete !== null && InlineQuizApp.onPostComplete.constructor === Function) {
            try {
                InlineQuizApp.onPostComplete();
            } catch (err) {
                d2log('onComplete Error!');
                d2log(err);
            }
...
InlineQuizApp.SetTextSlide = function(data, onOK, options) {
    ...
    if (options.nextTopic && (InlineQuizApp.onNextTopic !== null && InlineQuizApp.onNextTopic.constructor === Function)) {
        ILQ_buttonSet.appendChild(InlineQuizApp.MakeFullBaseButton(InlineQuizApp.onNextTopic, 'Next Topic'));
    }
